			...
			<div class="mdl-card__supporting-text">
				<form class="form" role="form" method="POST" action="{{ route('register') }}">
					...
					<div class="form-group{{ $errors->has('email') ? ' has-error' : '' }} mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
						...
						<div class="col-md-6">
							<input id="email" type="email" class="form-control mdl-textfield__input" name="email" maxlength="90" value="{{ old('email') }} " pattern="[\w._%+-]+@[\w._%+-]+.+ntnu.no" title="Use your @ntnu.no e-mail!" >
							<span class="mdl-textfield__error">
								Use your @ntnu.no e-mail! Check subdomain (@stud.ntnu.no) 
							</span>
							@if ($errors->has('email'))
								<span class="">
									{{ $errors->first('email') }}
